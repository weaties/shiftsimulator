...
class Polar:
    ...
    def from_dict(cls, d: Dict) -> "Polar":
        return cls(twa=list(d["twa"]), tws=list(d["tws"]),
                   table=[list(r) for r in d["table"]], name=d.get("name", "polar"))
...
_SHAPE_REL = [0, .45, .68, .76, .88, .95, 1.0, 1.02, 1.0, .97, .90, .78, .62, .52]
...
def synthetic_polar(
    ...
    """
    twa = list(range(0, 181, 5))
    shape = [_interp(a, _SHAPE_TWA, _SHAPE_REL) for a in twa]
    if pointing != 1.0:
        shape = [_pointing_adjust(a, s, pointing) for a, s in zip(twa, shape)]
    tws = list(tws_grid)
    table = []
    for a, s in zip(twa, shape):
