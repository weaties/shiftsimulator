...
from .metrics import BoatResult, ladder_gain_series, rank, summarize
...
def text_report(scenario: Scenario, states: List[BoatState]) -> str:
    ...
    lines.append("")
    hdr = f"{'#':>2}  {'boat':<22}{'finish':>9}{'tacks':>7}{'gybes':>7}" \
    ...
    if len(ranked) > 1 and ranked[0].finished and ranked[0].finish_time:
        ...
        win = ranked[0]
        for r in ranked[1:]:
            if r.finished and r.finish_time:
...
def replay_data(scenario: Scenario, states: List[BoatState]) -> Dict:
    ...
    for m in scenario.course.marks:
        xs.append(m.pos[0]); ys.append(m.pos[1])
    pad = 80.0
    bounds = {"minx": min(xs) - pad, "maxx": max(xs) + pad,
              "miny": min(ys) - pad, "maxy": max(ys) + pad}
    boats = []
    for b in states:
        boats.append({
            "name": b.cfg.name, "color": b.cfg.color,
            "finish_time": b.finish_time, "n_tacks": b.n_tacks, "n_gybes": b.n_gybes,
            ...
            "frames": [{"t": s.t, "x": s.pos[0], "y": s.pos[1], "hdg": s.heading,
                        "tack": s.tack, "man": s.maneuvering, "twd": s.twd,
                        "tws": s.tws, "spd": s.boat_speed, "lad": s.ladder} for s in b.history],
        })
    return {
        "name": scenario.name, "description": scenario.description,
        "ref_twd": scenario.ref_twd, "bounds": bounds,
...
def ladder_gain_svg(states: List[BoatState], path: str, reference: int = 0,
                    w: int = 720, h: int = 380) -> None:
    ...
    def X(t): return m + pw * (t / tmax if tmax else 0)
    def Y(v): return m + ph * (1 - (v - lo) / (hi - lo))
    ...
    for name, ys in series.items():
        pts = [f"{X(t):.1f},{Y(v):.1f}" for t, v in zip(times, ys) if v is not None]
        if pts:
            out.append(f'<polyline fill="none" stroke="{colors[name]}" '
                       f'stroke-width="2" points="{" ".join(pts)}"/>')
        out.append(f'<rect x="{m+pw-120}" y="{legend_y-9}" width="10" height="10" '
                   f'fill="{colors[name]}"/>')
...
def tracks_svg(scenario: Scenario, states: List[BoatState], path: str,
               w: int = 560, h: int = 720) -> None:
    ...
    def X(x): return m + (x - b["minx"]) * s
    def Y(y): return h - m - (y - b["miny"]) * s   # north is up

    out = _svg_open(w, h)
    out.append(f'<text x="{w/2}" y="20" text-anchor="middle" font-size="15" '
    ...
    ax, ay = w - 55, 55
    out.append(f'<line x1="{ax-wf[0]*22}" y1="{ay+wf[1]*22}" x2="{ax+wf[0]*22}" '
               f'y2="{ay-wf[1]*22}" stroke="#3a7" stroke-width="3" '
               f'marker-end="url(#a)"/>')
    out.append('<defs><marker id="a" markerWidth="8" markerHeight="8" refX="6" '
    ...
    for mk in scenario.course.marks:
        out.append(f'<circle cx="{X(mk.pos[0]):.1f}" cy="{Y(mk.pos[1]):.1f}" r="5" '
    ...
    for bo in states:
        ...
        out.append(f'<polyline fill="none" stroke="{bo.cfg.color}" stroke-width="1.6" '
