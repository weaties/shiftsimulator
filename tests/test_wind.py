...
from shiftsim.wind import (CompositeWind, OscillatingWind, PersistentShift,
                           PuffyWind, SteadyWind, wind_from_dict)


def test_oscillating_stays_within_amplitude():
...
def test_puffy_is_deterministic_by_seed():
    a = PuffyWind(seed=7); b = PuffyWind(seed=7); c = PuffyWind(seed=8)
    assert a.at(33, (0, 0)) == b.at(33, (0, 0))
    assert a.at(33, (0, 0)) != c.at(33, (0, 0))


def test_composite_layers_oscillation_on_trend():
    w = CompositeWind(fields=[
...
def test_roundtrip_dict():
    for w in (SteadyWind(twd=5, tws=9), OscillatingWind(amplitude=10),
              PersistentShift(total_shift=15), PuffyWind(seed=3)):
