...
from shiftsim.strategy import MinimizeTacks, Strategy
from shiftsim.wind import SteadyWind


def _boat(name: str, strat: Strategy, tack: str = "starboard") -> BoatConfig:
    return BoatConfig(name=name, polar=synthetic_polar(), strategy=strat, initial_tack=tack)


def test_heading_convention() -> None:
...
def test_boat_finishes_steady_wind() -> None:
    b = BoatState(cfg=_boat("m", MinimizeTacks()), pos=(0, 0), tack="starboard")
    simulate(
        [b], SteadyWind(twd=0, tws=10), windward_leeward(800, 0.0), 0.0, RunConfig(max_time=2000)
    )
    assert b.finished and b.finish_time is not None


def test_determinism() -> None:
    def run() -> list[tuple[float | None, int, int]]:
        sc = Scenario.load(
            os.path.join(os.path.dirname(__file__), "..", "scenarios", "oscillating_demo.json")
        )
...
def test_tack_on_header_beats_minimize_in_oscillation() -> None:
    sc = Scenario.load(
        os.path.join(os.path.dirname(__file__), "..", "scenarios", "oscillating_demo.json")
    )
...
def test_favoured_side_wins_persistent() -> None:
    sc = Scenario.load(
        os.path.join(os.path.dirname(__file__), "..", "scenarios", "persistent_demo.json")
    )
    res = {b.cfg.name: b for b in sc.run_sim()}
...
def test_header_strategies_quiet_in_steady_wind() -> None:
...
def test_half_header_finishes_and_beats_lift_in_oscillation() -> None:
    from shiftsim.strategy import TackAtHalfHeader, TackOnLift

    half = BoatState(
        cfg=_boat("half", TackAtHalfHeader(amplitude=12, fraction=0.5)),
        pos=(0, 0),
        tack="starboard",
    )
...
def test_all_boats_finish_demos() -> None:
