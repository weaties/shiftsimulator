...
OK = {
    ...
    "boats": [{"name": "A", "strategy": {"name": "minimize_tacks"},
               "polar": {"type": "synthetic"}}],
}


def _rejected(cfg) -> bool:
    try:
...
def test_rejects_no_boats():
    cfg = copy.deepcopy(OK); cfg["boats"] = []
    assert _rejected(cfg)


def test_rejects_too_many_boats():
    cfg = copy.deepcopy(OK)
    cfg["boats"] = [OK["boats"][0]] * (LIMITS["max_boats"] + 1)
    assert _rejected(cfg)


def test_rejects_tiny_dt():
    cfg = copy.deepcopy(OK); cfg["run"]["dt"] = 0.01
    assert _rejected(cfg)


def test_rejects_huge_max_time():
    cfg = copy.deepcopy(OK); cfg["run"]["max_time"] = LIMITS["max_time"] + 1
    assert _rejected(cfg)


def test_rejects_too_many_laps():
    cfg = copy.deepcopy(OK); cfg["course"]["laps"] = LIMITS["max_laps"] + 1
    assert _rejected(cfg)


def test_rejects_over_budget_combo():
