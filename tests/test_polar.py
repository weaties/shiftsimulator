...
def test_synthetic_speed_nonnegative_and_zero_head_to_wind():
    p = synthetic_polar()
    assert p.speed(0, 10) == 0.0           # can't sail straight into the wind
    assert p.speed(90, 10) > 0


def test_best_upwind_angle_realistic():
    p = synthetic_polar()
...
def test_best_downwind_angle_realistic():
    p = synthetic_polar()
...
def test_speed_increases_with_wind():
    p = synthetic_polar()
    assert p.speed(60, 6) < p.speed(60, 16)


def test_pointing_parameter_sharpens_upwind():
...
def test_csv_roundtrip():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        rows = [header] + [f"{a}," + ",".join(f"{v:.3f}" for v in row)
